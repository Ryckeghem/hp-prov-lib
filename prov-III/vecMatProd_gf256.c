                                              ...
                                              const uint8_t *B)
{
  ...
  __m256i T,T0,T4,w0,w1,w2,B0,B1,B2;
  unsigned int j;
...
{
  const __m256i mask_0f=_mm256_set1_epi8(15);
  __m256i T,T0,T4,w0,w1,w2,w3,w4,w5,B0,B1,B2,B3,B4,B5;
  unsigned int j;
                                       ...
                                       const uint8_t *B, const uint8_t *B_)
{
  ...
  for(j=0;j<NDIMR;++j)
  {
    ...
    /* here, _mm256_cmpgt_epi8 returns 0xff...ff<<(8*j) */
    w0^=_mm256_shuffle_epi8(T0,B0&mask_0f)&_mm256_cmpgt_epi8(mask_U,mask_0f);
    w3^=_mm256_shuffle_epi8(T0,B3&mask_0f)&_mm256_cmpgt_epi8(mask_U,mask_0f);
    w1^=_mm256_shuffle_epi8(T0,B1&mask_0f);
    w2^=_mm256_shuffle_epi8(T0,B2&mask_0f);
    w4^=_mm256_shuffle_epi8(T0,B4&mask_0f);
    w5^=_mm256_shuffle_epi8(T0,B5&mask_0f);
    w0^=_mm256_shuffle_epi8(T4,_mm256_srli_epi16(B0,4)&mask_0f)
        &_mm256_cmpgt_epi8(mask_U,mask_0f);
    w3^=_mm256_shuffle_epi8(T4,_mm256_srli_epi16(B3,4)&mask_0f)
        &_mm256_cmpgt_epi8(mask_U,mask_0f);
    w1^=_mm256_shuffle_epi8(T4,_mm256_srli_epi16(B1,4)&mask_0f);
    w2^=_mm256_shuffle_epi8(T4,_mm256_srli_epi16(B2,4)&mask_0f);
...
/*! \brief Vector-Umatrix product over GF(256): (1*v)*(v*v)=(1*v).
 ...
 \param[in] B An upper triangular matrix over GF(256) of size v*v.
 \req ceil(v/32)=4.
 \alloc 32v bytes for T_u, (v*(v+1))/2+PADDING32(v) bytes for B,
        v+PADDING32(v) bytes for w.
 \csttime T_u, B, w.
*/
                                             ...
                                             const uint8_t *B)
{
  ...
  __m128i e,S,w0,w1,u5,u00,u10,u20,u30,u40,u01,u11,u21,u31,u41,B0,B1,
          B00,B10,B01,B11;
  unsigned int i;
  ...
  B0=_mm_shuffle_epi8(B0,mask_rev64_epi16);
  /* * * * * * * B A --> 0 A 0 B 0 0 0 0 */
  u5=_mm_srli_epi32(_mm_slli_epi64(B1,56),8)
    |_mm_srli_epi32(_mm_slli_epi64(B1,48),24);
  /* even bytes *x^8 */
  ...
  for(i=0;i<NROWS;++i)
  {
    ...
    B1=_mm_unpacklo_epi8(B1,_mm_setzero_si128());
    /* each carry-less multiplication performs a dot product */
    w1^=_mm_clmulepi64_si128(u5,B1,0);
                                               ...
                                               const uint8_t *B)
{
  const __m128i mask_rev64_epi16=_mm_set_epi64x((uint64_t)0x9080b0a0d0c0f0e,
                                                (uint64_t)0x100030205040706);
  __m128i e,S,w0,w1,u00,u10,u20,u30,u40,u50,u60,u70,u01,u11,u21,u31,
          u41,u51,u61,u71,B0,B1,B00,B10,B01,B11,mask_U;
  unsigned int j;
  ...
  u60=_mm_slli_epi16(B1,8);
  /* odd bytes */
  u51=_mm_srli_epi16(B0,8);
  u61=_mm_srli_epi16(B1,8);

  B0=_mm_loadu_si128((__m128i*)u+6);
  /* 7 6 5 4 3 2 1 0 --> 1 0 3 2 5 4 7 6 */
  B0=_mm_shuffle_epi8(B0,mask_rev64_epi16);
  /* even bytes *x^8 */
  u70=_mm_slli_epi16(B0,8);
  /* odd bytes */
  u71=_mm_srli_epi16(B0,8);
  ...
  for(j=0;j<NDIMR;++j)
  {
    ...
    B1=_mm_loadu_si128((__m128i*)B+7);
    ...
    w0^=_mm_clmulepi64_si128(u70,B10,0);
    w0^=_mm_clmulepi64_si128(u70,B10,0x11);
    w1^=_mm_clmulepi64_si128(u61,B01,0);
    w1^=_mm_clmulepi64_si128(u61,B01,0x11);
    w1^=_mm_clmulepi64_si128(u71,B11,0);
    w1^=_mm_clmulepi64_si128(u71,B11,0x11);
  ...
  for(;j<(16+NDIMR);++j)
  {
    ...
    w0^=_mm_clmulepi64_si128(u60,B10,0);
    w0^=_mm_clmulepi64_si128(u60,B10,0x11);
    w1^=_mm_clmulepi64_si128(u51,B01,0);
    w1^=_mm_clmulepi64_si128(u51,B01,0x11);
    w1^=_mm_clmulepi64_si128(u61,B11,0);
    w1^=_mm_clmulepi64_si128(u61,B11,0x11);
    ...
    /* each carry-less multiplication performs a dot product */
    w0^=_mm_clmulepi64_si128(u70,B00,0);
    w0^=_mm_clmulepi64_si128(u70,B00,0x11);
    w1^=_mm_clmulepi64_si128(u71,B01,0);
    w1^=_mm_clmulepi64_si128(u71,B01,0x11);
