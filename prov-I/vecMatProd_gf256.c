...
/*! \brief Two vector-matrix products over GF(256): (1*v)*(v*o)=(1*o).
 \details One vector multiplies two matrices.
 \param[out] w A vector over GF(256) of size o, equal to u.B.
 \param[out] w_ A vector over GF(256) of size o, equal to u.B_.
 \param[in] u A vector over GF(256) of size v.
 \param[in] B A matrix over GF(256) of size v*o.
 \param[in] B_ A matrix over GF(256) of size v*o.
 \req ceil(o/32)=2.
 \alloc v bytes for u, v*o+PADDING32(o) bytes for B and B_,
        o+PADDING32(o) bytes for w and w_.
 \csttime B, B_.
 \vartime u.
*/
                                    ...
                                    const uint8_t *B, const uint8_t *B_)
{
  ...
  for(j=0;j<NROWS;++j)
  {
    ...
    B0=_mm256_loadu_si256((__m256i*)B);
    B1=_mm256_loadu_si256((__m256i*)B+1);
    B2=_mm256_loadu_si256((__m256i*)B_);
    B3=_mm256_loadu_si256((__m256i*)B_+1);
  ...
  _mm256_storeu_si256((__m256i*)w,w0);
  _mm256_storeu_si256((__m256i*)w+1,w1);
  _mm256_storeu_si256((__m256i*)w_,w2);
  _mm256_storeu_si256((__m256i*)w_+1,w3);
}
                                                 ...
                                                 const uint8_t *B)
{
  ...
  const __m256i mask_0f=_mm256_set1_epi8(15);
  __m256i T,T0,T4,w0,w1,B0,B1,sum,mask_U;
  unsigned int j;
...
/*! \brief Two vector-Umatrix products over GF(256): (1*o)*(o*o)=(1*o).
 \details One vector multiplies two upper triangular matrices.
 \param[out] w A vector over GF(256) of size o, equal to u.B.
 \param[out] w_ A vector over GF(256) of size o, equal to u.B_.
 \param[in] u A vector over GF(256) of size o.
 \param[in] B An upper triangular matrix over GF(256) of size o*o.
 \param[in] B_ An upper triangular matrix over GF(256) of size o*o.
 \req ceil(o/32)=2.
 \alloc o bytes for u, (o*(o+1))/2+PADDING32(o) bytes for B and B_,
        o+PADDING32(o) bytes for w and w_.
 \csttime B, B_.
 \vartime u.
*/
                                       ...
                                       const uint8_t *B, const uint8_t *B_)
{
  ...
  const __m256i mask_01=_mm256_set1_epi8(1);
  const __m256i mask_0f=_mm256_set1_epi8(15);
  __m256i T,T0,T4,w0,w1,w2,w3,B0,B1,B2,B3,sum,sum_,mask_U;
  unsigned int j;
  ...
  for(j=0;j<NDIMR;++j)
  {
    ...
    sum=_mm256_shuffle_epi8(T0,B0&mask_0f);
    sum_=_mm256_shuffle_epi8(T0,B2&mask_0f);
    w1^=_mm256_shuffle_epi8(T0,B1&mask_0f);
    w3^=_mm256_shuffle_epi8(T0,B3&mask_0f);
    sum^=_mm256_shuffle_epi8(T4,_mm256_srli_epi16(B0,4)&mask_0f);
    sum_^=_mm256_shuffle_epi8(T4,_mm256_srli_epi16(B2,4)&mask_0f);
    w1^=_mm256_shuffle_epi8(T4,_mm256_srli_epi16(B1,4)&mask_0f);
    w3^=_mm256_shuffle_epi8(T4,_mm256_srli_epi16(B3,4)&mask_0f);
    /* here, _mm256_cmpgt_epi8 returns 0xff...ff<<(8*j) */
    w0^=sum&_mm256_cmpgt_epi8(mask_U,mask_0f);
    w2^=sum_&_mm256_cmpgt_epi8(mask_U,mask_0f);
  ...
  for(;j<NDIM;++j)
  {
    ...
    T=_mm256_loadu_si256((__m256i*)multab_x0_x4_gf256+*u);
    B1=_mm256_loadu_si256((__m256i*)B);
    B3=_mm256_loadu_si256((__m256i*)B_);

    T0=_mm256_permute4x64_epi64(T,0x44);
    T4=_mm256_permute4x64_epi64(T,0xee);

    sum=_mm256_shuffle_epi8(T0,B1&mask_0f);
    sum_=_mm256_shuffle_epi8(T0,B3&mask_0f);
    sum^=_mm256_shuffle_epi8(T4,_mm256_srli_epi16(B1,4)&mask_0f);
    sum_^=_mm256_shuffle_epi8(T4,_mm256_srli_epi16(B3,4)&mask_0f);
    w1^=sum&_mm256_cmpgt_epi8(mask_U,mask_0f);
    w3^=sum_&_mm256_cmpgt_epi8(mask_U,mask_0f);
  ...
  w1^=_mm256_loadu_si256((__m256i*)w);
  w3^=_mm256_loadu_si256((__m256i*)w_);
  _mm256_storeu_si256((__m256i*)w,w1);
  _mm256_storeu_si256((__m256i*)w_,w3);
}
...
/*! \brief Vector-Umatrix product over GF(256): (1*v)*(v*v)=(1*v).
 ...
 \req ceil(v/32)=3.
 \alloc v bytes for u, (v*(v+1))/2+PADDING32(v) bytes for B,
        v+PADDING32(v) bytes for w.
 \csttime B.
 \vartime u.
*/
                                                 ...
                                                 const uint8_t *B)
{
  ...
  const __m256i mask_01=_mm256_set1_epi8(1);
  const __m256i mask_0f=_mm256_set1_epi8(15);
  __m256i T,T0,T4,w0,w1,w2,B0,B1,B2,sum,mask_U;
  unsigned int j;
...
/*! \brief Two vector-Umatrix products over GF(256): (1*v)*(v*v)=(1*v).
 \details One vector multiplies two upper triangular matrices.
 \param[out] w A vector over GF(256) of size v, equal to u.B.
 \param[out] w_ A vector over GF(256) of size v, equal to u.B_.
 \param[in] u A vector over GF(256) of size v.
 \param[in] B An upper triangular matrix over GF(256) of size v*v.
 \param[in] B_ An upper triangular matrix over GF(256) of size v*v.
 \req ceil(v/32)=2.
 \alloc v bytes for u, (v*(v+1))/2+PADDING32(v) bytes for B and B_,
        v+PADDING32(v) bytes for w and w_.
 \csttime B, B_.
 \vartime u.
*/
                                       ...
                                       const uint8_t *B, const uint8_t *B_)
{
  ...
  const __m256i mask_01=_mm256_set1_epi8(1);
  const __m256i mask_0f=_mm256_set1_epi8(15);
  __m256i T,T0,T4,w0,w1,w2,w3,w4,w5,B0,B1,B2,B3,B4,B5,sum,sum_,mask_U;
  unsigned int j;
  ...
  w2=_mm256_setzero_si256();
  w3=_mm256_setzero_si256();
  w4=_mm256_setzero_si256();
  w5=_mm256_setzero_si256();
  /* columns of u, rows of B */
  mask_U=_mm256_loadu_si256((__m256i*)Tmask_U);
  for(j=0;j<NDIMR;++j)
  {
    ...
    B2=_mm256_loadu_si256((__m256i*)B+2);
    B3=_mm256_loadu_si256((__m256i*)B_);
    B4=_mm256_loadu_si256((__m256i*)B_+1);
    B5=_mm256_loadu_si256((__m256i*)B_+2);
    ...
    w5^=_mm256_shuffle_epi8(T0,B5&mask_0f);
    sum^=_mm256_shuffle_epi8(T4,_mm256_srli_epi16(B0,4)&mask_0f);
    sum_^=_mm256_shuffle_epi8(T4,_mm256_srli_epi16(B3,4)&mask_0f);
    w1^=_mm256_shuffle_epi8(T4,_mm256_srli_epi16(B1,4)&mask_0f);
    w2^=_mm256_shuffle_epi8(T4,_mm256_srli_epi16(B2,4)&mask_0f);
    w4^=_mm256_shuffle_epi8(T4,_mm256_srli_epi16(B4,4)&mask_0f);
    w5^=_mm256_shuffle_epi8(T4,_mm256_srli_epi16(B5,4)&mask_0f);
    /* here, _mm256_cmpgt_epi8 returns 0xff...ff<<(8*j) */
    w0^=sum&_mm256_cmpgt_epi8(mask_U,mask_0f);
    w3^=sum_&_mm256_cmpgt_epi8(mask_U,mask_0f);
  ...
  _mm256_storeu_si256((__m256i*)w+2,w2);
  _mm256_storeu_si256((__m256i*)w_,w3);
  _mm256_storeu_si256((__m256i*)w_+1,w4);
  _mm256_storeu_si256((__m256i*)w_+2,w5);
  w+=NDIMR;
  w_+=NDIMR;
  ...
  for(;j<(32+NDIMR);++j)
  {
    ...
    B1=_mm256_loadu_si256((__m256i*)B);
    B2=_mm256_loadu_si256((__m256i*)B+1);
    B4=_mm256_loadu_si256((__m256i*)B_);
    B5=_mm256_loadu_si256((__m256i*)B_+1);

    T0=_mm256_permute4x64_epi64(T,0x44);
    T4=_mm256_permute4x64_epi64(T,0xee);

    sum=_mm256_shuffle_epi8(T0,B1&mask_0f);
    sum_=_mm256_shuffle_epi8(T0,B4&mask_0f);
    w2^=_mm256_shuffle_epi8(T0,B2&mask_0f);
    w5^=_mm256_shuffle_epi8(T0,B5&mask_0f);
    sum^=_mm256_shuffle_epi8(T4,_mm256_srli_epi16(B1,4)&mask_0f);
    sum_^=_mm256_shuffle_epi8(T4,_mm256_srli_epi16(B4,4)&mask_0f);
    w2^=_mm256_shuffle_epi8(T4,_mm256_srli_epi16(B2,4)&mask_0f);
    w5^=_mm256_shuffle_epi8(T4,_mm256_srli_epi16(B5,4)&mask_0f);
    w1^=sum&_mm256_cmpgt_epi8(mask_U,mask_0f);
    w4^=sum_&_mm256_cmpgt_epi8(mask_U,mask_0f);
  ...
  for(;j<NDIM;++j)
  {
    ...
    T=_mm256_loadu_si256((__m256i*)multab_x0_x4_gf256+*u);
    B2=_mm256_loadu_si256((__m256i*)B);
    B5=_mm256_loadu_si256((__m256i*)B_);

    T0=_mm256_permute4x64_epi64(T,0x44);
    T4=_mm256_permute4x64_epi64(T,0xee);

    sum=_mm256_shuffle_epi8(T0,B2&mask_0f);
    sum_=_mm256_shuffle_epi8(T0,B5&mask_0f);
    sum^=_mm256_shuffle_epi8(T4,_mm256_srli_epi16(B2,4)&mask_0f);
    sum_^=_mm256_shuffle_epi8(T4,_mm256_srli_epi16(B5,4)&mask_0f);
    w2^=sum&_mm256_cmpgt_epi8(mask_U,mask_0f);
    w5^=sum_&_mm256_cmpgt_epi8(mask_U,mask_0f);
  ...
  w1^=_mm256_loadu_si256((__m256i*)w);
  w2^=_mm256_loadu_si256((__m256i*)w+1);
  w4^=_mm256_loadu_si256((__m256i*)w_);
  w5^=_mm256_loadu_si256((__m256i*)w_+1);
  _mm256_storeu_si256((__m256i*)w,w1);
  _mm256_storeu_si256((__m256i*)w+1,w2);
  _mm256_storeu_si256((__m256i*)w_,w4);
  _mm256_storeu_si256((__m256i*)w_+1,w5);
}
...
#define NDIM2 NB_OIL
#if NDIM1&31
  #define NDIM1R (NDIM1&31)
#else
  #define NDIM1R 32
#endif

/*! \brief Vector-matrix product over GF(256):
 (1*v)*(v*v)||(1*v)*(v*o)=(1*(v+o)).
 \param[out] w A vector over GF(256) of size v+o, equal to u.U||u.B.
 \param[in] u A vector over GF(256) of size v.
 \param[in] U An upper triangular matrix over GF(256) of size v*v.
 \param[in] B A matrix over GF(256) of size v*o.
 \req ceil(v/32)=3.
 \req ceil(o/32)=2.
 \alloc v bytes for u, (v*(v+1))/2+PADDING32(v) bytes for U,
        v*o+PADDING32(o) bytes for B, v+o+PADDING32(o) bytes for w.
 \csttime U, B.
 \vartime u.
*/
                                      ...
                                      const uint8_t *U, const uint8_t *B)
{
  ...
  for(j=0;j<NDIM1R;++j)
  {
    /* T=Tl Th, T0=Tl Tl, T4=Th Th */
    T=_mm256_loadu_si256((__m256i*)multab_x0_x4_gf256+*u);
    B0=_mm256_loadu_si256((__m256i*)U);
    B1=_mm256_loadu_si256((__m256i*)U+1);
    B2=_mm256_loadu_si256((__m256i*)U+2);
  ...
  for(;j<(32+NDIM1R);++j)
  {
    /* T=Tl Th, T0=Tl Tl, T4=Th Th */
    T=_mm256_loadu_si256((__m256i*)multab_x0_x4_gf256+*u);
    B1=_mm256_loadu_si256((__m256i*)U);
    B2=_mm256_loadu_si256((__m256i*)U+1);
    B3=_mm256_loadu_si256((__m256i*)B);
    B4=_mm256_loadu_si256((__m256i*)B+1);
  ...
  for(;j<NDIM1;++j)
  {
    /* T=Tl Th, T0=Tl Tl, T4=Th Th */
    T=_mm256_loadu_si256((__m256i*)multab_x0_x4_gf256+*u);
    B2=_mm256_loadu_si256((__m256i*)U);
    B3=_mm256_loadu_si256((__m256i*)B);
    B4=_mm256_loadu_si256((__m256i*)B+1);
  ...
  _mm256_storeu_si256((__m256i*)w+3,w4);
}
...
/*! \brief Two vector-matrix products over GF(256): (1*v)*(v*o)=(1*o).
 ...
 \param[in] B_ A matrix over GF(256) of size v*o.
 \req ceil(o/32)=2.
 \alloc 32v bytes for T_u, v*o+PADDING32(o) bytes for B and B_,
        o+PADDING32(o) bytes for w and w_.
 \csttime T_u, B, B_, w, w_.
*/
...
/*! \brief Two vector-Umatrix products over GF(256): (1*v)*(v*v)=(1*v).
 ...
 \param[in] B_ An upper triangular matrix over GF(256) of size v*v.
 \req ceil(v/32)=3.
 \alloc 32v bytes for T_u, (v*(v+1))/2+PADDING32(v) bytes for B and B_,
        v+PADDING32(v) bytes for w and w_.
 \csttime T_u, B, B_, w, w_.
*/
                                             ...
                                             const uint8_t *B)
{
  ...
  __m128i e,S,w0,w1,u00,u10,u20,u30,u01,u11,u21,u31,B0,B1,B00,B10,B01,B11;
  unsigned int i;
  ...
  /* zero padding */
  B1=_mm_srli_si128(_mm_slli_si128(B1,7),7);
  /* 7 6 5 4 3 2 1 0 --> 1 0 3 2 5 4 7 6 */
  B0=_mm_shuffle_epi8(B0,mask_rev64_epi16);
  B1=_mm_shuffle_epi8(B1,mask_rev64_epi16);
  /* even bytes *x^8 */
