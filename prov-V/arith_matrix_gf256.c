...
static void symmetrizeUpperT_gf256_sse2(uint8_t *A)
{
  ...
  /* for each 8*16 block of the lower triangular part */
  for(j=0;j<i;++j)
  {
    /* compute the transpose of a block and add it to the corresponding block
       of the upper triangular part */
  ...
  /* update the lower part of At */
  At6^=_mm_unpackhi_epi64(_mm_slli_si128(_mm_srli_si128(s2,11),15),
                          _mm_setzero_si128());
  At4^=_mm_unpackhi_epi64(_mm_slli_si128(_mm_srli_si128(s2,1),13),
                          _mm_setzero_si128());
  At5^=_mm_unpackhi_epi64(_mm_slli_si128(_mm_srli_si128(s2,6),14),
                          _mm_setzero_si128());
  At0^=_mm_unpacklo_epi64(s3,_mm_setzero_si128());
  At2^=_mm_unpacklo_epi64(_mm_slli_si128(_mm_srli_si128(s0,3),3),
                          _mm_setzero_si128());
  At1^=_mm_slli_si128(_mm_srli_si128(s3,10),2);
  At3^=_mm_slli_si128(_mm_srli_si128(s0,12),4);
  _mm_storeu_si128((__m128i*)At,At0);
  _mm_storeu_si128((__m128i*)(At+NDIM),At1);
  _mm_storeu_si128((__m128i*)(At+2*NDIM),At2);
  _mm_storeu_si128((__m128i*)(At+3*NDIM),At3);
  ...
  /* rectangular blocks, contigue on the lower triangular part */
  At=A+(NDIM-(NDIM&7))*NDIM;
  A_=A+NDIM-(NDIM&7);
  for(j=0;j<(NDIM>>4);++j)
  {
    At0=_mm_loadu_si128((__m128i*)At+j);
    At1=_mm_loadu_si128((__m128i*)(At+NDIM)+j);
    At2=_mm_loadu_si128((__m128i*)(At+2*NDIM)+j);
    At3=_mm_loadu_si128((__m128i*)(At+3*NDIM)+j);

    r0=_mm_unpacklo_epi8(At0,At1);
    r1=_mm_unpackhi_epi8(At0,At1);
    r2=_mm_unpacklo_epi8(At2,At3);
    r3=_mm_unpackhi_epi8(At2,At3);

    _mm_storeu_si128((__m128i*)tmp,_mm_unpacklo_epi16(r0,r2));
    _mm_storeu_si128((__m128i*)tmp+1,_mm_unpackhi_epi16(r0,r2));
    _mm_storeu_si128((__m128i*)tmp+2,_mm_unpacklo_epi16(r1,r3));
    _mm_storeu_si128((__m128i*)tmp+3,_mm_unpackhi_epi16(r1,r3));
    for(i=0;i<16;i+=2)
    {
      ((uint32_t*)A_)[i*(NDIM>>2)]^=tmp[i];
      ((uint32_t*)A_)[(i+1)*(NDIM>>2)]^=tmp[i+1];
    }
    A_+=NDIM<<4;
  }
  /* rectangular block 4*8 of the lower triangular part */
  At0=_mm_loadu_si64((__m128i*)At+j);
  At1=_mm_loadu_si64((__m128i*)(At+NDIM)+j);
  At2=_mm_loadu_si64((__m128i*)(At+2*NDIM)+j);
  At3=_mm_loadu_si64((__m128i*)(At+3*NDIM)+j);
...
void sqrMatrixToUpperT_gf256_avx2(uint8_t *Au, uint8_t *A)
{
  ...
  for(i=0;i<NDIMR;++i)
  {
    ...
    _mm256_storeu_si256((__m256i*)Au+3,_mm256_loadu_si256((__m256i*)A+3));
    A+=NDIM+1;
    Au+=NDIM-i;
  }
  for(;i<(32+NDIMR);++i)
                                                    ...
                                                    const uint8_t *B)
{
  ...
  __m256i T,T0,T4,C0,C1,C2,C3,B0,B1,B2,B3;
  const uint8_t *Bj;
  unsigned int i,j;
  ...
  for(i=0;i<NROWS;++i)
  {
    ...
    C3=_mm256_loadu_si256((__m256i*)C+3);
    ...
    for(j=i;j<NROWS;++j)
    {
      ...
      B3=_mm256_loadu_si256((__m256i*)Bj+3);
      ...
      C3^=_mm256_shuffle_epi8(T0,B3&mask_0f);
      ...
      C3^=_mm256_shuffle_epi8(T4,_mm256_srli_epi16(B3,4)&mask_0f);
      ++A;
      Bj+=NCOLS;
    }
    ...
    _mm256_storeu_si256((__m256i*)C+3,C3);
    B+=NCOLS;
    C+=NCOLS;
  }
  _mm256_storeu_si256((__m256i*)C,C0);
