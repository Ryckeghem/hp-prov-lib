              ...
              unsigned long long mlen, const unsigned char *sk)
{
  ...
  uint8_t h[NB_EQ+PADDING8(NB_EQ)],P1v[NB_EQ+PADDING8(NB_EQ)],
          vec_v[NB_VIN+PADDING32(NB_VIN)],vec_o[NB_OIL+PADDING32(NB_OIL)],
                  ...
                  unsigned long long mlen, const unsigned char *esk)
{
  ...
  unsigned char prefix;
  uint8_t h[NB_EQ+PADDING8(NB_EQ)],P1v[NB_EQ+PADDING8(NB_EQ)],
          vec_v[NB_VIN+PADDING32(NB_VIN)],v_rev[NB_VIN+PADDING16(NB_VIN)];
                       ...
                       const uint8_t *P1, const uint8_t *P2, const uint8_t *P3)
{
  ...
  uint8_t t[NB_EQ],vec_o[NB_OIL+PADDING32(NB_OIL)],
          s_rev[NB_VAR+PADDING16(NB_VAR)];
  #if PADDING32(NB_OIL)>=PADDING16(NB_VAR)
    uint8_t vec_n[NB_VAR+PADDING32(NB_OIL)];
  #else
    uint8_t vec_n[NB_VAR+PADDING16(NB_VAR)];
  #endif
  ...
  {
    /* v^T *P1 */
    vecMatProd_Uv_mulTab_gf256_avx2(vec_n,s,P1+i*LEN_P1);
    /* v^T *P2 */
    vecMatProd_mulTab_gf256_avx2(vec_n+NB_VIN,s,P2+i*LEN_P2);
    /* o^T *P3 */
