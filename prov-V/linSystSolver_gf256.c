...
static void elimStep4_cstTime_gf256_avx2(uint8_t *A, uint8_t *row,
                                                     unsigned int j)
{
  ...
  __m256i A0,T,T0,T4,r0,r1,r2,r3,B00,B01,B10,B11,B20,B21,B30,B31;
  unsigned int i;
  ...
  for(i=0;i<NROWS;++i)
  {
    ...
    r3=_mm256_loadu_si256((__m256i*)A+3);
...
int gaussJordanElim_cstTime_gf256_avx2(uint8_t *A, uint8_t *b)
{
  ...
  __m256i pr0,pr1,pr2,pr3,A0,A1,A2,A3,b1,m_pivot,m_nopivot,i8,nb_pivots;
  uint64_t b0;
  unsigned int i,j;
  ...
  for(j=0;j<NCOLSR_Ab;++j)
  {
    ...
    for(i=0;i<NROWS;++i)
    {
      ...
      pr3^=_mm256_loadu_si256((__m256i*)(A+i*SIZE_ROW_Av+96))&b1;
    }
    ...
    _mm256_storeu_si256((__m256i*)pivot_row+3,pr3);

    /* elimination of the column by the pivot */
    /* A[i]^=A[i][j]*(pivot_row*pivot_row[j]^(-1)) for each row */
    elimStep4_cstTime_gf256_avx2(A,pivot_row,j);
    ...
    pr3=_mm256_loadu_si256((__m256i*)pivot_row+3);
    i8=_mm256_setzero_si256();
    ...
    #endif
    {
      ...
      A3=_mm256_loadu_si256((__m256i*)(A+i*SIZE_ROW_Av+96));
      ...
      _mm256_storeu_si256((__m256i*)(A+i*SIZE_ROW_Av+96),A3);
    }
  ...
  for(j=0;j<31;++j)
  {
    ...
    i8=_mm256_setzero_si256();
    /* note that necessarily, nb_pivots<=j */
    #if (94+NCOLSR_Ab)<NROWS
    for(i=0;i<=(j+64+NCOLSR_Ab);++i)
    #elif (64+NCOLSR_Ab)>=NROWS
    for(i=0;i<NROWS;++i)
    #else
    for(i=0;i<=(((j+64+NCOLSR_Ab)<NROWS)?j+64+NCOLSR_Ab:NROWS-1);++i)
    #endif
    {
      /* we store i+1 and nb_pivots+1 in this implementation */
      i8=_mm256_add_epi8(i8,mask_01);
  ...
  for(i=0;i<NCOLSR;++i)
  {
    ...
    A0|=_mm256_loadu_si256((__m256i*)A+3);
